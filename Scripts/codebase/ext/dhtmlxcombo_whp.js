...
dhtmlXCombo.prototype.enableOptionAutoHeight = function (a, b) {
    if (!this._listHeightConf) this._listHeightConf = this.DOMlist.style.height == "" ? 100 : parseInt(this.DOMlist.style.height); arguments.length == 0 && (a = 1); this.autoHeight = convertStringToBoolean(a); var c = this; if (this.autoHeight) {
        var d = function () { window.setTimeout(function () { c._setOptionAutoHeight(a, b) }, 1) }; this.ahOnOpen = this.attachEvent("onOpen", d); if (!this.awOnOpen) this.ahOnXLE = this.attachEvent("onXLE", d); var e; this.ahOnKey = this.attachEvent("onKeyPressed",
function () { this._filter && (e && window.clearTimeout(e), window.setTimeout(function () { c.DOMlist.style.display == "block" && c._setOptionAutoHeight(a, b) }, 50)) })
    } else typeof this.ahOnOpen != "undefined" && (this.detachEvent(this.ahOnOpen), this.ahOnXLE && this.detachEvent(this.ahOnXLE), this.ahOnKey && this.detachEvent(this.ahOnKey), this.setOptionHeight(this._listHeightConf))
};
